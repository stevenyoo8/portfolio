...
#  Description: Utilize adjacency matrices to bucket fill (re-color) images
#  Date: July 19, 2023
...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        for i in range(len(self.nodes)):
            for j in range(len(self.nodes)):
                ...
                if i != j and j in self.nodes[i].edges:
                    print("1", end="")
                else:
                    print("0", end="")
            print()  # new line
    ...
    def bfs(self, start_index, color):
        ...
        while not queue.is_empty():
            current_node_index = queue.dequeue()
            current_node = self.nodes[current_node_index]

            if not current_node.visited:
                current_color = current_node.color
                queue_colors_to_change = Queue()
                queue_colors_to_change.enqueue(current_node_index)

                while not queue_colors_to_change.is_empty():
                    node_to_change_index = queue_colors_to_change.dequeue()
                    node_to_change = self.nodes[node_to_change_index]

                    if node_to_change.color == current_color:
                        node_to_change.visit_and_set_color(color)
                        self.print_image()
                        for edge_index in node_to_change.edges:
                            queue_colors_to_change.enqueue(edge_index)
    ...
    def dfs(self, start_index, color):
        ...
        while not stack.is_empty():
            ...
            if not current_node.visited:
                # mark current node as visited and change color
                current_node.visit_and_set_color(color)
                self.print_image()

                # iterate through edges in reverse
                for neighbor_index in reversed(current_node.edges):
                    neighbor_node = self.nodes[neighbor_index]
                    if neighbor_node.color == current_node.prev_color and not neighbor_node.visited:
